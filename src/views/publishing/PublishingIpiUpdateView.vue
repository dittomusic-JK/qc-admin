<template>
  <div>
    <PageHeader title="IPI/CAE Numbers Update" description="Newly assigned IPI/CAE numbers awaiting confirmation with societies." />

    <DataTable
      :columns="columns"
      :rows="publishingIpiUpdates"
      row-key="id"
    >
      <template #toolbar>
      ...
      <template #cell-newValue="{ row }">
        <span class="tabular-nums font-medium">{{ row.newValue }}</span>
      </template>
...
import { publishingIpiUpdates } from '../../data/mockQc'
